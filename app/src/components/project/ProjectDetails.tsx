      ...
      <div className={card}>
        ...
        <div className="divide-y divide-gray-100">
          ...
          {overviewRows.map(([label, field, type, sym]) => (
            <div key={field} className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-4 py-2">
      ...
      <div className={card}>
        ...
        <div className="mt-3 border border-gray-200 rounded-lg overflow-hidden">
          {leftOverRows.map(([label, field, type, sym]) => (
            <div key={field} className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-4 px-3 py-2 odd:bg-yellow-50 even:bg-yellow-50 border-b border-gray-100 last:border-0">
              <span className="flex-1 text-xs text-gray-600">{label}</span>
              <SymInput field={field} type={type} sym={sym} formData={formData} setFormField={setFormField} />
            </div>
          ))}
              ...
              {moqRows.map((row) => {
                ...
                <div key={row.id} className="space-y-1">
                  <div className="grid grid-cols-3 gap-3 items-center">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setMoqRows((prev) => prev.map((r) =>
                          r.id === row.id ? { ...r, individualUnits: v, moq: v } : r
                        ));
                      }}
                      placeholder="0"
                    ...
                    <input
                      type="number"
                      value={row.unitsPerInner}
                      onChange={(e) => updateMoqRow(row.id, "unitsPerInner", e.target.value)}
                      placeholder="0"
                  ...
                  {rowErr?.unitsPerInner && (
                    <p className="text-[0.6rem] text-red-500 font-medium">{rowErr.unitsPerInner}</p>
                  )}
