...
export default function QuotePage() {
  const { summaryRows, summaryTableRows, ppuUnits, allMoqResults, perMoqSummaryRows, computeForQty, formData, setFormField, moqRows } = useProject();
  ...
  // tracks which field the user last edited per row: "margin" or "ppu"
  const [moqLastEdited, setMoqLastEdited] = useState<Record<number, "margin" | "ppu">>({});

  // Use the selected MOQ's summary rows; fall back to the global summaryRows
  ...
  // Sort unique MOQs (same pack) ascending for bracket detection
  ...
  // Build a margin map for the PDF generator. PPU-entered rows back-calculate margin.
  ...
  return (
    <main className="min-h-screen flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-auto px-6 py-6 w-full">
        ...
        {/* ── Page header ── */}
        <div className="mb-5">
          {/* Row 1: title + date/buffer controls */}
          ...
          {/* Row 2: export buttons right-aligned */}
          <div className="flex items-center justify-end gap-2">
            <button
              onClick={handlePreview}
              disabled={generating || allMoqResults.length === 0}
              className="flex items-center gap-2 bg-[#e8473f] hover:bg-[#d43f37] disabled:opacity-50 text-white text-xs font-semibold px-4 h-7 transition-colors"
            >
              <FileText size={12} />
            ...
            <button className="flex items-center gap-1.5 border border-gray-200 bg-white hover:bg-gray-50 text-gray-600 text-xs font-medium px-3 h-7 transition-colors">
              <div className="w-3 h-3 relative shrink-0"><Image src={workdriveLogo} alt="WorkDrive" fill className="object-contain" /></div>
              WorkDrive
            </button>
            <button className="flex items-center gap-1.5 border border-gray-200 bg-white hover:bg-gray-50 text-gray-600 text-xs font-medium px-3 h-7 transition-colors">
              <div className="w-3 h-3 relative shrink-0"><Image src={crmLogo} alt="CRM" fill className="object-contain" /></div>
              CRM
            </button>
          </div>
        </div>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {allMoqResults.length === 0 || allMoqResults.every(r => r.totalCustomerPrice === 0) ? (
                ...
                // Derived display values for the non-authoritative input
                const derivedMarginDisplay = lastEdited === "ppu" && hasPpu
                  ? adjMarginPct.toFixed(2)
                  : marginStr;
                const derivedPpuDisplay = lastEdited === "margin" && hasMargin
                  ? adjPPU.toFixed(4)
                  : ppuStr;

                const hasAdjustment = hasMargin || hasPpu;
                      ...
                      <div className="flex items-center justify-center gap-0.5">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            // clear the PPU field so derived value shows
                      ...
                      <div className="flex items-center justify-center gap-0.5">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            // clear the margin field so derived value shows
        ...
        <div className="border border-gray-100 rounded-sm overflow-hidden">
          <div className="bg-gray-50 border-b border-gray-100 px-3 py-2 flex items-center gap-2">
            ...
            <button
              ...
              className="ml-auto flex items-center gap-1.5 h-6 px-3 text-[0.65rem] font-semibold bg-[#e8473f] hover:bg-[#d43f37] disabled:opacity-40 text-white transition-colors whitespace-nowrap"
            >
              <FileText size={11} />
          ...
          <div className="p-4">
            <div className="flex items-end gap-3 mb-4 flex-wrap">
              ...
              {/* Two-way adjuster — both inputs always visible */}
              <div>
