...
export default function ColumnsSection({
  ...
  // If the targeted column is a derived column (negative id / sourceId set),
  // we also remove the source column to avoid orphaned references.
  const removeColumnById = (id: number) => {
    setColumns((prev) => {
      if (prev.length <= 1) return prev;
      // If id exists directly in raw columns, remove it
      if (prev.some((c) => c.id === id)) return prev.filter((c) => c.id !== id);
      // Otherwise it's a derived column — find it from the effectiveColumns
      // prop (via closure) and remove its source column from raw state instead.
      const derived = columns.find((c) => c.id === id);
      if (derived?.sourceId != null) return prev.filter((c) => c.id !== derived.sourceId);
      return prev;
    });
  };
